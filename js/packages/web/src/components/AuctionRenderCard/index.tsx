...
export const AuctionRenderCard = (props: AuctionCard) => {
  ...
  const art = useArt(id);
  // const creators = useCreators(auctionView)s;
  const name = art?.title || ' ';
  const creatorDeatil =
    auctionDetail.prismicContent &&
    auctionDetail.prismicContent.length > 0 &&
    auctionDetail.prismicContent[0].data.creator.length > 0 &&
    auctionDetail.prismicContent[0].data.creator.filter(
      x => x.creator_id[0].text === auctionView.auctionManager.authority,
    );
